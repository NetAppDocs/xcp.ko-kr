...
[role="lead"]
NFS `help` Command 명령 명령 목록, 명령 매개 변수 및 각 매개 변수에 대한 간략한 설명을 표시합니다. 를 클릭합니다 `help` 명령은 XCP 도구를 처음 사용하는 초보자에게 유용합니다.
...
를 사용합니다 `info` 매개 변수 `help` 설명서, 예제 및 튜닝 권장 사항을 표시하는 명령입니다.
